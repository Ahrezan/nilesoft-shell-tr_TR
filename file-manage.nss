...
{
	menu(separator="after" title='Yolu kopyala' image=icon.copy_path)
	{
		item(where=sel.count > 1 title='Kopyala (@sel.count) öğe seçili' cmd=command.copy(sel(false, "\n")))
	...
	item(mode="single" type="file" title="Uzantıyı değiştir" image=\uE0B5 cmd=if(input("Uzantıyı değiştir", "Yeni uzantıyı girin"),
		io.rename(sel.path, path.join(sel.dir, sel.file.title + "." + input.result))))

	menu(separator="after" image=\uE290 title='Seçimi yönet')
	{
		item(title="Tümünü seç" image=icon.select_all cmd=command.select_all)
		item(title="Tersini seç" image=icon.invert_selection cmd=command.invert_selection)
		item(title="Seçimi kaldır" image=icon.select_none cmd=command.select_none)
	}
	...
	menu(title="Göster/Gizle" image=icon.show_hidden_files)
	{
		...
		item(title="Dosya uzantıları" image=icon.show_file_extensions cmd='@command.toggleext')
	}
	...
	menu(type='file|dir|back.dir' mode="single" title='Öznitelikler' image=icon.properties)
	{
		...
		separator
		item(title="Oluşturulma" keys=io.dt.created(sel.path, 'y/m/d') cmd=io.dt.created(sel.path,2000,1,1) vis=label)
		item(title="Değiştirilme" keys=io.dt.modified(sel.path, 'y/m/d') cmd=io.dt.modified(sel.path,2000,1,1) vis=label)
		item(title="Erişim" keys=io.dt.accessed(sel.path, 'y/m/d') cmd=io.dt.accessed(sel.path,2000,1,1) vis=label)
	}

	menu(mode="single" type='file' find='.dll|.ocx' separator="before" title='Sunucu Kaydı' image=\uea86)
	{
	...
	item(where=!wnd.is_desktop title='Klasör seçenekleri' image=icon.folder_options cmd=command.folder_options)
}
